...
matlist gradw(mat x, mat y, mat z, double h) {
...
matlist pairwiseseparation(mat ri, mat rj) {
...
mat acceleration(mat pos, mat vel, double m, double h, double k, double l, double n, double nu) {
...
sph sph_alloc(double dt, double h, double k, double n, double nu, double *m) {
	sph m = malloc(sizeof(sph));
	m->dt = dt;
	m->h = h;
	m->k = k;
	m->l = 2.01;
